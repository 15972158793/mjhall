gameclass.zjhtableGold = gameclass.zjhtablefk.extend({
    //计时器
    _timerControl: null,
    //搓牌按钮
    _cuoPai: null,
    ...
    showgameinfo: function () {
        ...
        ccui.helper.seekWidgetByName(this.node, "extraReward").setVisible(true);
        if (this.mod_niuniu.gameniuniuinfo != null) {
            this.genpai.setString("封顶:20000");

            ccui.helper.seekWidgetByName(this.node, "quanbi").setString("全比");
            ccui.helper.seekWidgetByName(this.node, "bishuangbei").setVisible(true);
            ccui.helper.seekWidgetByName(this.node, "bishuangbei").setString("比牌双倍");
        }

        var roomNameNode = ccui.helper.seekWidgetByName(this.node, "roomnum");
        if (this.mod_niuniu.isJingDian()) {
            this.menpai.setVisible(false);
            this.bipai1.setVisible(false);
            roomNameNode.setString(staticString.classic + staticString.gamePszName);
        } else {
            this.menpai.setVisible(true);
            this.bipai1.setVisible(true);

            this.menpai.setString("闷:" + (this.mod_niuniu.gameniuniuinfo.round < this.mod_niuniu._canKanRound ? this.mod_niuniu.gameniuniuinfo.round + 1 : this.mod_niuniu._canKanRound) + "/" + this.mod_niuniu._canKanRound);
            this.bipai1.setString("比:" + (this.mod_niuniu.gameniuniuinfo.round < this.mod_niuniu._canBiRound ? this.mod_niuniu.gameniuniuinfo.round + 1 : this.mod_niuniu._canBiRound) + "/" + this.mod_niuniu._canBiRound);

            roomNameNode.setString(staticString.crazy + staticString.gamePszName);

        }
    },
    refreshStep: function () {
        if(this.mod_niuniu.gameniuniuinfo){
            this.curround.setString("第" + (this.mod_niuniu.gameniuniuinfo.round + 1) + "/20轮");
        }else{
            this.curround.setString("第1/20轮");
        }
    },
    ...
    splipChip:function (bets) {
        var compareArr = gameclass.zjhtableGold.chipSourceArr;
        var useNumArr = [];
 ...
 * 回收筹码到赢家
 */
gameclass.zjhtablefk.prototype.recoverChip = function(p) {
...
gameclass.zjhtablefk.prototype.runJuShuAction = function() {
...
gameclass.zjhtablefk.prototype.getMaxChip = function ($leftArr, leftNum) {
    var leftArr = [];
    var useNum = 0;
    var isGet = false;
 ...
 * 开始和换桌按钮显示监测
 */
gameclass.zjhtablefk.prototype.startBtnVisibleCheck = function () {
    if(!this._isPlayEnd) {
        return false;
    }
 ...
 * 操作按钮可见性处理
 */
gameclass.zjhtablefk.prototype.btnVisibleCheck = function () {
    if(!this._isPlayEnd) {
        return false;
    }
    ...
    if (this.mod_niuniu.gameniuniuinfo == null) {
        ...
        var canbipai;
        if (this.mod_niuniu.gameniuniuinfo.round >= this.mod_niuniu._canBiRound) {
            canbipai = true;
        }
        var cankanpai = false;
        if (this.mod_niuniu.gameniuniuinfo.round >= this.mod_niuniu._canKanRound) {
            cankanpai = true;
        }
        ...
        // cc.log(begin+","+!isGameEnd+","+!mineUserData.discard+","+!mineUserData.open+","+canopenqipai+","+cankanpai+","+!mineUserData.lose);
...
gameclass.zjhtableGold.prototype.switchPoit = function (point) {
    return parseInt(point / this.bottomScore);
}
/**
 * 开局打底处理
 */
gameclass.zjhtableGold.prototype.startBetHandle = function () {
 ...
 * 播放结束动画，播放完毕，弹出结算面板
 */
gameclass.zjhtableGold.prototype.playEndMovie = function () {
    this.isBiPaiing = false;
    var _this = this;
 ...
 * 皇冠动画，以及金币加减动画
 */
gameclass.zjhtableGold.prototype.playEndMovieSub = function () {
    var _this = this;
    var len = this.mod_niuniu.gameniuniuinfo.info.length;
    for (var i = 0; i < len; i++) {
        var info = this.mod_niuniu.gameniuniuinfo.info[i];
...
gameclass.zjhtableGold.prototype.createCrownHead = function (parent, index) {
...
gameclass.zjhtableGold.prototype.goldFiy = function () {
    var text;
    var len = this.mod_niuniu.gameniuniuinfo.info.length;
    for (var i = 0; i < len; i++) {
        var info = this.mod_niuniu.gameniuniuinfo.info[i];
 ...
 */
gameclass.zjhtableGold.prototype.layCardList = function (cardContain, centerIndex, dis, isBack) {
    var singleW;
 ...
 * @param card
 */
gameclass.zjhtableGold.prototype.createCardList = function (cardArr) {
    var cardContain = new cc.Sprite();
 ...
 * @return {*}
 */
gameclass.zjhtableGold.prototype.createCard = function (card) {
    var abcd = ["a", "d", "b", "c"];
 ...
 * 更新用户金币显示
 */
gameclass.zjhtableGold.prototype.updatePersonGold = function () {
...
gameclass.zjhtableGold.prototype.updateDifen = function () {
    this.danzhu.setString("底分:" + this.bottomScore);
};
/**
 * 可用筹码资源
 * @type {[number,number,number,number,number,number,number]}
 */
gameclass.zjhtableGold.chipSourceArr = [50, 100, 200, 500, 1000, 2000, 5000, 10000];
