...
gameclass.leopardKingTable = gameclass.baseui.extend({
    node:null,
    chairNodeArr:null,
    btn_bets:null,
    btn_sendBets:null,
    ...
    show:function(){
        ...
        this.node = this.game.uimgr.createnode(res.leopardKingTable,true);
        ...
        this.clock.setPosition(1136/2,320);
    ...
    updateroominfo:function(data){
        ...
        if(this.gameType == 40000){
            this.betsArr = [100,500,1000,5000,10000];
        }else{
            this.betsArr = [100,200,500,1000,2000];
        }
        //选择筹码按钮
        var _this = this;
    ...
    initMyInfo:function(total){
        ...
        gameclass.mod_base.showtximg(head,this.mod_king.logindata.imgurl, 0, 0,"im_headbg5", false);
    ...
    initZhuangInfo:function(zhuangInfo){
        ...
        gameclass.mod_base.showtximg(ccui.helper.seekWidgetByName(this.node,"zhuangIcon"),
            zhuangInfo.uid?zhuangInfo.head:"res/im_headbg8.png", 0, 0,"im_headbg5", false);
    ...
    reflashZhuangInfo:function(){
        ...
        gameclass.mod_base.showtximg(ccui.helper.seekWidgetByName(this.node,"zhuangIcon"),
            data.uid?data.head:"res/im_headbg8.png", 0, 0,"im_headbg5", false);
            ...
            _this.zhong.runAction(cc.sequence(cc.spawn(cc.moveTo(0.3,cc.p(568,320)),cc.scaleTo(0.3,1,1)),cc.callFunc(function(){
                ...
                _this.node.scheduleOnce(function(){
                    ...
                    for(var i = 0;i < 3;i++){
                        ...
                        var spsezi = node.getChildByTag(2000+i);
                        var endPos = cc.p(0.2*node.getContentSize().width + (0.27*i*node.getContentSize().width),node.getContentSize().height/2);
                        if(spsezi){
                    ...
                    var total = data.result[0] + data.result[1]+data.result[2];
    ...
    getRewardIndex:function(arr){
        ...
        if(totalNum <= 17 && totalNum > 10){
            if(!(arr[0] == arr[1] && arr[1] == arr[2])){
                rewardArr.push(2);
            }
        }
        if(totalNum > 3){
            rewardArr.push(totalNum-1);
        }
    ...
    playBetAnim:function(){
        ...
        this.clock.setCompleteListener(function(){
            ...
            _this.zhong.y = 320;
            _this.zhong._scale = 2.5;
            _this.zhong.runAction(cc.spawn(cc.moveTo(0.5,cc.p(586,567)),cc.scaleTo(1,1,1)));
    ...
    initShow:function(){
        ...
        for(var i = 0;i < this.betsLayer.getChildrenCount();i++){
            ...
            var sp = null;
            if(i==0){
                sp = new cc.Sprite(res.blueImg);
            }else if(i == 1){
                sp = new cc.Sprite(res.greenImg);
            }else if(i ==2){
                sp = new cc.Sprite(res.redImg);
            }else{
        ...
        gameclass.createbtnpress(this.node, "btn_more", function () {
            _this.game.uimgr.showui("bzwRecordUi").setBaseInfo(_this.recordArr);
        });
    ...
    setBetImg:function(){
        for(var i = 0;i < 5;i++){
            if(i == this.selectBet){
                this.btn_bets[i].loadTextureNormal(res.betSlectImg,ccui.Widget.LOCAL_TEXTURE);
            }else{
                this.btn_bets[i].loadTextureNormal(res.betNormalImg,ccui.Widget.LOCAL_TEXTURE);
            }
...
var wzui = gameclass.baseui.extend({
    ctor: function () {
        this._super();
    },
    show: function () {
...
var szui = gameclass.baseui.extend({
    ctor: function () {
        this._super();
    },
    show: function () {
        this.node = this.game.uimgr.createnode(res.bzwSZ, true);
        this.addChild(this.node);
    },
...
var bzwRecordUi = gameclass.baseui.extend({
    ctor: function () {
        this._super();
    },
    show: function () {
        this.node = this.game.uimgr.createnode(res.bzwRecord, true);
        this.addChild(this.node);
        ...
        ccui.helper.seekWidgetByName(this.node,"Panel_1").addTouchEventListener(function(sender,type){
            if(type != ccui.Widget.TOUCH_ENDED) return;
            _this.game.uimgr.closeui("bzwRecordUi");
        })
    },
    setBaseInfo:function(data){
        ...
        for(var i = 0;i < data.length;i++){
            ...
            listCell.getChildByName("type").setTexture(res["record" + data[i].type]);
        }
    },
})
