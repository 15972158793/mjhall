...
gameclass.pdk_wildcardType = gameclass.tool_cardType.extend({
    razznum:0,
    razz:0,
    currazz:[],
    ctor:function () {
    ...
    checkW:function(cardsArr,handCard,wildCard){
        ...
        var funcs = [
            ...
            this.isSanDaiW.bind(this),
            this.isSiDaiW.bind(this),
    ...
    isSanDaiW:function(cards,size,wildCard){
        if(size < 3) return 0;
        var arr = [];
        var res = this.isSanDai(cards,size);
        if(res){
            arr.push(res);
        }
        res = this.matchCard(cards,size,wildCard,this.isSanDaiW.bind(this),this.isSanDai.bind(this));
        if(res){
            arr = arr.concat(res) ;
        }
        return arr.length > 0 ? arr : 0;
    },
    isSiDaiW:function(cards,size,wildCard){
        if(size < 6) return 0;
        var arr = [];
        var res = this.isSanDai(cards,size);
        if(res){
        ...
        res = this.matchCard(cards,size,wildCard,this.isSiDaiW.bind(this),this.isSiDai.bind(this));
        if(res){
            arr = arr.concat(res) ;
        }
        return arr.length > 0 ? arr : 0;
...
gameclass.tool_wildcardType.create = function(){
    ...
    return new gameclass.pdk_wildcardType();
}
/*
